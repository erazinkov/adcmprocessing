...
EnergyPeakFinder::EnergyPeakFinder() : calib_{1.0}, offset_{0.0}
{
    ...
    findPeakPosFunctions_ = {
        ...
        {EnergyPeak::Id::SILICON, [&](TH1 *hist, TH1 *) { return findPeakPosSilicon(hist, 1.35); }},
        {EnergyPeak::Id::CARBON, [&](TH1 *hist, TH1 *) { return findPeakPosCarbon(hist, 1.35); }},
        {EnergyPeak::Id::OXYGENADD, [&](TH1 *hist, TH1 *) { return findPeakPosOxygenAdd(hist, 1.5); }},
        {EnergyPeak::Id::OXYGEN, [&](TH1 *hist, TH1 *) { return findPeakPosOxygen(hist, 1.5); }},
...
//    for (size_t i{0}; i < peaks.size(); i++) {
//        auto peakPos{findPeakPosFunctions_.at(peaks.at(i).id())(hist, histRc)};
//        std::cout << std::fixed << std::setprecision(2) << peakPos.pos << "\u00B1" << peakPos.err << " ";
//    }
//    std::cout << std::endl;
    for (size_t i{0}; i < peaks.size(); i++) {
        auto peakPos{findPeakPosFunctions_.at(peaks.at(i).id())(hist, histRc)};
        std::cout << std::fixed << std::setprecision(2) << peakPos.pos << " ";
    }
    std::cout << std::endl;

}

void EnergyPeakFinder::processExternal(TH1D *hist, TH1D * histRc)
{
    energyPeaks_.clear();
    fCalib_->SetParameters(0.0, 1.0, 0.0);
    TVirtualFitter::SetDefaultFitter("Minuit");
...
void EnergyPeakFinder::processExternal(const std::vector<EnergyPeak> &energyPeaksInternal, std::vector<EnergyPeak> &energyPeaksExternal)
{
    energyPeaks_.clear();
    TGraphErrors graphPolN;
    for (size_t i{0}, j{0}; i < energyPeaksInternal.size(); i++) {
        auto id{energyPeaksInternal.at(i).id()};
        auto it{
            std::find_if(energyPeaksExternal.begin(), energyPeaksExternal.end(),
                         [id](const EnergyPeak& p) { return p.id() == id; })
        };
        if (it != energyPeaksExternal.end()) {
            graphPolN.SetPoint(j, (*it).channel(), energyPeaksInternal.at(i).channel());
            std::cout << i << " " << j << " " <<  (*it).channel() << " " << energyPeaksInternal.at(i).channel() << std::endl;
            j++;
        }
    }
...
EnergyPeakFinder::PeakPos EnergyPeakFinder::findPeakPosFerrum847(TH1 *h, const double A)
{
...
EnergyPeakFinder::PeakPos EnergyPeakFinder::findPeakPosFerrum1238(TH1 *h, const double A)
{
...
EnergyPeakFinder::PeakPos EnergyPeakFinder::findPeakPosHydrogen(TH1 *h, const double A)
{
...
EnergyPeakFinder::PeakPos EnergyPeakFinder::findPeakPosSilicon(TH1 *h, const double A)
{
...
EnergyPeakFinder::PeakPos EnergyPeakFinder::findPeakPosCarbon(TH1 *h, const double A)
{
...
EnergyPeakFinder::PeakPos EnergyPeakFinder::findPeakPosOxygen(TH1 *h, const double A)
{
...
EnergyPeakFinder::PeakPos EnergyPeakFinder::findPeakPosOxygenAdd(TH1 *h, const double A)
{
    double peakEnergyAdd{EnergyPeak::energyById(EnergyPeak::Id::OXYGENADD)};
    double posAdd{fCalib_->GetX(peakEnergyAdd)};
    double sigmaAdd{fCalib_->GetX(TMath::Sqrt(peakEnergyAdd) * A)};
    ...
    return {f.GetParameter(1) - (pos - posAdd), f.GetParError(1)};
}

EnergyPeakFinder::PeakPos EnergyPeakFinder::findPeakPosFerrum7631(TH1 *h, const double A)
{
